<template>
  <header class=" h-24 w-full z-20 sticky top-0 font-fuzzy">
    ...
    >
      <!-- logo -->
      <nuxt-link to="/" class="flex justify-center lg:col-span-1 h-full text-white hover:text-black duration-100">
    ...
    <nav :class="{hidden:isActive}" class="lg:hidden w-screen absolute top-0 h-screen backdrop-blur-sm bg-black/30 z-40 grid grid-cols-2 grid-rows-6 ">
      <div class="grid grid-cols-2 grid-rows-1 col-span-2  my-4 h-12 justify-items-center w-full">
        ...
        <div class="w-full h-full pr-8 lg:hidden flex justify-end  " @click="toggle">
          <i class="fas fa-bars h-8 w-8 z-50 place-self-center text-white hover:text-yellow-500 duration-100" />
        </div>
      </div>
      <!-- liens -->
      <div v-for="lien in liens" :key="lien.texte" class="mb-4 py-4 flex flex-col items-center">
        <nuxt-link
          key="Index"
          :to="lien.lien"
          class="text-2xl font-medium text-white hover:text-black hover:bg-yellow-500 hover:rounded-full p-2 duration-100"
          @click.native="toggle"
        >
          {{ lien.texte }}
        </nuxt-link>
